...
define((['Sheet', 'commonLogic']),function(Sheet, _) {
	...
	function render (that) {
		...
		var strCurrDate = _.formatDate(startDate, 'yyyy-MM-dd');
		if( operateDate !== '' && operateDate >= strCurrDate) {
			...
			currCloseTime.setMinutes(currCloseTime.getMinutes() - ELAPSED_MINUTES);

			if( startDate > currCloseTime) {
				// 如果当前时间已经超过截止送餐时间
				if( !(startDate.getHours() >=0 && startDate.getHours() < 5)) {
					// 并且当前时间还没有到凌晨,例如现在是23:30左右,网点结束时间是00:00,那么开始日期为下一天
