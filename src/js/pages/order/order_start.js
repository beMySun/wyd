...
require([ 'DateSelector', 'remote', 'Sheet', 'Dialog', 'Toast', "fastClick", 'commonLogic', 'BMap', 'swiper' ], function(DateSelector, remote, Sheet,
		Dialog, Toast, FastClick, _) {
	...
	function makeAppointDate(closeTime, operateDate) {
		if (closeTime) {
			var currDate = new Date();
			var strCurrDate = _.formatDate(currDate, 'yyyy-MM-dd');
			if (operateDate !== '' && operateDate >= strCurrDate) {
				// 对于还没有开始送餐的网点,以第一天为初始值
				return operateDate;
			}

			var currCloseTime = _.parseTime(closeTime);
			...
			currCloseTime.setMinutes(currCloseTime.getMinutes() - 75);

			if (currDate > currCloseTime) {
				if (!(currDate.getHours() >= 0 && currDate.getHours() < 5)) {
					currDate.setDate(currDate.getDate() + 1);
				}
			}
			return _.formatDate(currDate, 'yyyy-MM-dd');
		}
	...
	function init(requestData) {
		...
		requestData['appointDate'] = makeAppointDate(requestData['closeTime'], requestData['operateDate']);
		...
		$('.order_location').on('click', function() {
			if (!addressListView) {
				addressListView = new AddressListView(DATA['addressId'] && 'address' + DATA['addressId'], DATA['addresses'], function(addressData) {
					...
					var appointDate = makeAppointDate(requestData['closeTime'], requestData['operateDate']);
					if (appointDate !== requestData['appointDate']) {
						setAppointDateStr(appointDate, requestData['appointDate']);
						requestData['appointDate'] = appointDate; // 在定位后的推荐订餐日期
					}
		...
		$('.order_appointdate').on('click', function() {
			...
			// $(".dialog_box").removeClass().addClass("dialog_box
			// date_select");
			dateSelector.show();
		...
		function onConfirm() {
			...
			/* 作为临时系统并行跳转处理，全部上线后可以放开
			if (requestData['storeId'] !== 6 && requestData['storeId'] !== 7) {
				Dialog.show({
		...
		function createAddressNewView() {
			if (!addressNewView) {
				addressNewView = new AddressNewView(function(mapData) {
					...
					var addressHtml = '<div id="address' + mapData['addressId'] + '" class="wui_cell address_item active">'
							+ '<div class="wui_cell_lt">' + '<i class="btn_list_select_normal"></i>' + '</div>'
							+ '<div class="wui_cell_ct wui_cell_primary ct_text_rows">' + '<p class="upper_title address_street">'
							+ mapData['location'] + mapData['detailAddress'] + '</p>' + '<p class="side_title color_999 address_door">'
							+ mapData['cityName'] + '</p>' + '</div>' + '</div>';
					...
					if (localSearchBottom.getStatus() == BMAP_STATUS_SUCCESS) {
						...
						$.each(results, function(index, result) {
							for (var i = 0; i < result.getCurrentNumPois(); i++) {
								newDiv += '<div lng="' + result.getPoi(i).point.lng + '" lat="' + result.getPoi(i).point.lat
										+ '" class="location_item">' + '<span class="upper_title">' + result.getPoi(i).title + '</span>'
										+ '<span class="side_text">' + result.getPoi(i).address + '</span>' + '</div>';
							}
						});
				...
				footer : [
						...
						},
						{
							...
							handler : function() {
								...
								var html = "";
								if (currCity == "广州市") {
									pics = [ "http://7xs4xd.com2.z0.glb.qiniucdn.com/TianHeBei_1.jpg",
											"http://7xs4xd.com2.z0.glb.qiniucdn.com/PanYu_1.jpg",
								...
								if (!storeMapSheet) {
									...
									$storeMaps.innerHTML = '<div class="btn_closeMap" style="z-index:10;"></div>' + '<div class="swiper-container">'
											+ '<div class="swiper-wrapper" id="ctrl_maps_content"></div>' + '<div class="swiper-pagination"></div>'
											+ '</div>';
