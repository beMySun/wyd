...
require(['jquery', 'Dialog', 'remote', 'CartView', 'DateSelector', 'TimeSelector', 'commonLogic', 'fastClick'], function ($, Dialog, Remote, CartView, DateSelector, TimeSelector, _, FastClick) {
    ...
    var timeSelector = new TimeSelector({
        ...
        handler: function (selectTime) {
            ...
            Remote.checkStoreOrderLimit(DATA["appointDate"], selectTime, function(res){
                //TODO
                if(!res){
                    Dialog.show({
                        content: '非常抱歉，您选择的“用餐时间”我们暂时没法准时送达，请电话联系客服预约下单，客服热线：<a href="tel:400-8765-313">400-8765-313</a> 谢谢谅解。',
                        footer: [{
                            position: 'center',
                            text: '换个时间',
                            handler: function(){
    ...
    $(".mui-checkbox-con").on('click',function () {
        if(isChecked("#deliver_sooner")){
            console.log("checked");
        }
        else {
