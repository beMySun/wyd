...
define(([ 'jquery', 'Toast', 'Dialog', 'commonLogic' ]), function($, Toast, Dialog, _) {
	...
	var commitProductsData = function(productData, callback){
		post('/order/orderProducts', productData, '正在保存...', callback);
	}
	...
	// 根据指定日期时间，检查能否进单
	var checkStoreOrderLimit = function(appointDate, appointTime ,callback){
		var data = {appointDate : appointDate,appointTime:appointTime};
		post("/order/limit/check", data, "请稍侯...", callback);
	}

	// 取消订单，微信端暂时用于用户取消微信支付时需要取消已经生成的订单
	var cancelOrder = function(orderId, callback, errorCallback){
		post("/order/"+ orderId +"/cancel", {}, "请稍侯...", callback, errorCallback);
	}
	...
	return {
		...
		checkStoreOrderLimit:   checkStoreOrderLimit,
